...
// Assuming your FrameRate struct is in this namespace
...
namespace EventHorizon.Tests
{
	...
	public class FrameRateTests
	{
		...
		public void Constructor_WithNegativeNumerator_ShouldThrowArgumentException() =>
			Assert.Throws<ArgumentException>(() => _ = new FrameRate(-30));
		...
		public void Constructor_WithZeroNumerator_ShouldThrowArgumentException() =>
			Assert.Throws<ArgumentException>(() => _ = new FrameRate(0));
			...
			double expectedValue)
		{
			...
			Assert.AreEqual(expectedValue, frameRate.GetAsDouble(), $"Failed for frame rate {numerator}/{denominator}");
		}
		...
		{
			var frameRate = new FrameRate(numerator, denominator);
			Assert.AreEqual(expectedValue, frameRate.GetFrameDuration(),
				$"Failed for frame rate {numerator}/{denominator}");
		}
		...
		{
			var frameRate = new FrameRate(24);
		...
		{
			var frameRate1 = new FrameRate(30);
			var frameRate2 = new FrameRate(30);
		...
		{
			var frameRate1 = new FrameRate(30);
			var frameRate2 = new FrameRate(24);
		...
		public void Simplify_CorrectlySimplifiesFraction(int beforeNum, int beforeDen, int expectedNumerator,
			int expectedDenominator)
		{
			var original = new FrameRate { numerator = beforeNum, denominator = beforeDen };
			var reduced = original.Simplify();
		...
		public void TryParse_WithProperInput_ReturnsValidFrameRate(string input, int expectedNumerator,
			int expectedDenominator)
		{
			var result = FrameRate.TryParse(input, out var frameRate);
